...
const onOrderPay = async () => {
  ...
  // 正式环境 微信支付
    ...
    const res = await getPayWxPayMiniPayAPI({ orderId: query.id }) // 返回订单支付信息
    // 微信支付API 需求参数：{appId、nonceStr、package、paySign、signType、timeStamp}
      ...
      <view class="overview" :style="{ paddingTop: safeAreaInsets!.top + 20 + 'px' }">
        ...
        <template v-if="order.orderState === OrderState.DaiFuKuan">
          ...
          <view class="tips">
            ...
            <uni-countdown color="#fff" splitorColor="#fff" :showDay="false" :showColon="false"
              :second="order.countdown" @timeup="onTimeup" />
          </view>
