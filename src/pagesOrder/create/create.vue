...
const selectedAddress = computed(() => {
  return addressStore.selectedAddress?.id ? addressStore.selectedAddress : orderPre.value?.userAddresses.find(item => item.isDefault)
})

// 提交订单
const onOrderSubmit = async () => {
  if (!selectedAddress.value?.id) return uni.showToast({ icon: "none", title: "请填写收货地址" })
